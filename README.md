...
- **Почта:** Unisender Go — `SMTP_*`, `MAIL_FROM`, `OWNER_EMAIL` (см. `.env.example`)
- **Render:** те же `SMTP_*` + `UNISENDER_API_KEY` (= SMTP_PASS) — см. раздел ниже
...
| Почта | Nodemailer + Unisender Go SMTP (локально) / HTTP API (Render) |
...
- **Локально:** Nodemailer → Unisender Go SMTP (`smtp.go2.unisender.ru:587`)
- **На Render (Free):** Unisender Go HTTP API — SMTP-порт 587 заблокирован платформой
- Обработка ошибок: `ApiExceptionFilter`, коды 400 / 503 / 500 / 429

### Почта: Unisender Go

Сервис: [Unisender Go](https://go2.unisender.ru/ru/settings/smtp-configuration)
...
**Render Free** ([документация](https://render.com/docs/free)) блокирует SMTP-порт 587. На проде backend автоматически переключается на **HTTP API** Unisender Go.
...
| `SMTP_PORT` | `587` |
| `SMTP_SECURE` | `false` |
| `SMTP_USER` | ваш числовой логин из Unisender Go |
| `SMTP_PASS` | пароль из Unisender Go |
| `UNISENDER_API_KEY` | **тот же пароль**, что `SMTP_PASS` |
| `UNISENDER_API_URL` | `https://go2.unisender.ru/ru/transactional/api/v1` |
| `MAIL_FROM` | подтверждённый email |
| `OWNER_EMAIL` | email для заявок |
| `NODE_ENV` | `production` |
...
- Настройка Unisender Go: SMTP `smtp.go2.unisender.ru:587`, подтверждение `MAIL_FROM`
- Render Free: автопереключение на HTTP API Unisender Go (SMTP заблокирован)
