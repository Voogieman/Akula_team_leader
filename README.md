...
## Для проверяющего — как проверить форму на демо

На демо письма отправляются через **sandbox-домен Unisender Go**. Это тестовый режим: письма можно доставлять **только на адреса из списка «Подтверждённые email-адреса»** в личном кабинете Unisender Go.
...
3. Заполните остальные поля (комментарий — не короче 10 символов) и нажмите «Отправить».
4. Должны прийти **2 письма**: заявка на `OWNER_EMAIL` и копия на указанный email.

**Можно ли добавить другие почты, не только одну?**

Да. В Unisender Go → **Учётная запись → Подтверждённые email-адреса** можно добавить **до 10 адресов** (включая email владельца аккаунта). Каждый новый адрес нужно подтвердить по ссылке из письма — после этого форма на демо будет работать и с этим email.

| Режим | Кто может получить письмо |
|-------|---------------------------|
| **Демо (sandbox)** | Только адреса из списка подтверждённых в Unisender Go |
| **Production (свой домен)** | Любой email, который пользователь вводит в форму |

Подробнее о переходе на production — раздел [Демо и production (почта)](#демо-и-production-почта).
...
|------------|--------|-----|
| Секция «Обо мне» (стек, опыт, направления) | ✅ | `#about` |
| Секция «Как работаю» + использование AI | ✅ | `#workflow` |
| Секция «Кейсы / проекты» со ссылками на GitHub | ✅ | `#projects` |
| Секция «Контакты» | ✅ | `#contact` |
| Форма: имя, телефон, email, комментарий | ✅ | `POST /api/contact` |
| 2 письма (владельцу + копия пользователю) | ✅ | `MailService` |
| UI: loading / success / error | ✅ | форма + AI-блок |
| Frontend: HTML, SCSS, TypeScript | ✅ | `client/` |
| Адаптивная вёрстка | ✅ | mobile-first, бургер-меню |
| Backend API (NestJS) | ✅ | `server/src/` |
| AI-интеграция в продукте | ✅ | `#ai-helper`, `POST /api/ai/summary` |
| README: запуск, стек, форма, AI, что с ИИ / вручную | ✅ | этот файл |
| Ссылка на GitHub | ✅ | [Voogieman/Akula_team_leader](https://github.com/Voogieman/Akula_team_leader) |
| Деплой (демо) | ✅ | [akula-team-leader.onrender.com](https://akula-team-leader.onrender.com) |
...
2. **Подтверждённые email-адреса** → добавьте нужные адреса (до 10 шт., каждый — по ссылке из письма)
3. В `.env` / Render:
...
> Sandbox: письма можно отправлять **только на подтверждённые адреса** (до 10 в кабинете Unisender Go), включая email в поле формы.
...
### Демо и production (почта)

Для тестового задания на демо сознательно используется **sandbox** Unisender Go — это позволяет проверить полный цикл формы (UI → API → 2 письма) без покупки домена.

| | **Демо (сейчас)** | **Production** |
|---|-------------------|------------------|
| Отправитель | `test@sandbox-….unigosendbox.com` | `noreply@ваш-домен.ru` |
| Получатели | только из списка **«Подтверждённые email»** в Unisender Go | **любой** email из формы |
| Заявка владельцу | ✅ на `OWNER_EMAIL` | ✅ |
| Копия пользователю | ✅ только если email подтверждён в Unisender | ✅ на любой адрес |
| Render Free | HTTP API Unisender (SMTP 587 заблокирован) | то же или SMTP на платном Render |

**Почему на демо форма «работает только с одним email»:** sandbox не пропускает письма на неподтверждённые адреса — это ограничение платформы Unisender, а не ошибка формы.

**Что сделать для production:**

1. Unisender Go → **Домены отправки** → добавить и подтвердить **свой домен** (DNS: SPF, DKIM)
2. Убрать `UNISENDER_SANDBOX_DOMAIN`, задать `MAIL_FROM=noreply@ваш-домен.ru`
3. `OWNER_EMAIL` — ваш рабочий ящик; копии уходят на **любой** email из поля формы

Логика backend (2 письма, валидация, состояния UI) при этом **не меняется** — меняются только переменные окружения и настройки в Unisender Go.
...
- Настройка Unisender Go: sandbox на демо, в README описан переход на свой домен в production
